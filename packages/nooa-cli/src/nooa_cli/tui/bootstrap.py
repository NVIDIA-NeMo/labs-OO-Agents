...
def session_options_from_config(config: Config):
    """Translate terminal configuration at the native host boundary."""
    from nooa_cli.interactive.options import SessionOptions

    values = config.tui.model_dump(include=set(SessionOptions.model_fields))
    values.update(
        working_dir=str(config.agent.working_dir),
        summarization=config.agent.summarization,
        legacy_agent=config.legacy_agent,
    )
    return SessionOptions(**values)
        ...
        session_manager = (
            ...
            if resume_id is not None
                ...
                agent_cls=(
                    ...
                    else (config.tui.agent_spec or "ExperimentalCodingAgent")
                ),
                working_dir=str(config.agent.working_dir),
            )
        )
...
def build_registry(result: BootstrapResult, frontend: Frontend) -> CommandRegistry:
    from nooa_cli.interactive.options import configure_session_skills

    from .commands import CommandRegistry

    options = session_options_from_config(result.config)
