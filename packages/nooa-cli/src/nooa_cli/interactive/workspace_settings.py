...
class WorkspaceSettings(Skill):
    ...
    def status(self) -> dict[str, Any]:
        ...
        """
        from .memory import tui_agent_memory_key

        options = self._options()
        key = tui_agent_memory_key(self._agent, options.policy_config())
        reflection = getattr(self._agent, "_tui_reflection_runner", None)
        return {
    ...
    async def _run_control(self, name: str, args: list[str]) -> str:
        ...
        from .memory import configure_tui_memory
        ...
        def configure_memory():
            ...
            configure_tui_memory(
                ...
                agent_db=getattr(session, "agent_db_path", getattr(session, "path", None)),
