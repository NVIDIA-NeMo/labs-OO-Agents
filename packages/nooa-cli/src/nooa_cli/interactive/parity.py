...
def prepare_sessions(source: Path, output: Path) -> dict:
    """Copy a stopped session under its exclusive ownership lock.
    ...
    with SessionStore(source.parent).open(source.stem) as handle:
        ...
        try:
            ...
            try:
                ...
                backup.close()
                connection.close()
            shutil.copyfile(target, pool / source.name)
            ...
            manifest = {
                ...
                "sha256": hashes,
                "shared_resources": "Workspace files and user configuration are shared.",
            }
