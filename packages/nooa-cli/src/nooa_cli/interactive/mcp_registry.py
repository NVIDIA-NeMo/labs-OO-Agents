...
class MCPRegistry(Skill):
    ...
    **The ``<mcp>`` context block is the single source of truth for what is
    ...
    the interactive host requires a user-level approval for the fingerprint of the complete
    ...
    invalidate approval. Only after the exact fingerprint matches does the interactive host resolve
    ...
    **Timeout / no-hang guarantee.** ``connect`` bounds the OAuth wait with
    ...
    def _config_path(self) -> Path:
        ...
        from nooa_cli.coding.settings import _SETTINGS_FILENAME as SETTINGS_FILENAME
    ...
    def __init__(
        ...
        Args:
            ...
            watch_settings: Reload layered interactive host settings before lifecycle commands.
                The production interactive host enables this so agent-assisted config edits
    ...
    def refresh_settings(self) -> list[str]:
        """Reload inline MCP definitions from the layered interactive host settings.

        The agent-assisted ``/mcp-add`` workflow edits ``settings.yaml`` while
        the interactive host is already running. Production registries opt into this refresh
        ...
        from nooa_cli.coding.settings import (
    ...
    def _bind_oauth_code_prompt(self, callback: Callable[[str], Awaitable[str]] | None) -> None:
        """Bind the host interactive host's thread-safe manual OAuth input bridge."""
    ...
    def discovered(self) -> list[str]:
        ...
        Returns an empty list if the ``mcp`` package is unavailable (the interactive host
    ...
    async def connect(
        ...
        **kwargs: Any,
        ...
        if unexpected:
            ...
            raise TypeError(
                "Connection-time MCP endpoint/config overrides are not allowed by the interactive host "
