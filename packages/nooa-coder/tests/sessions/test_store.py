...
def test_list_none_returns_all_sessions(tmp_path):
    store = SessionStore(tmp_path)
    for index in range(25):
        store.create(session_id=f"session-{index}").close()
...
def test_concurrent_liveness_probes_never_self_collide(tmp_path):
    ...
    reports a healthy, never-opened session as falsely "active". A shared
...
def test_liveness_probe_does_not_break_a_real_opener(tmp_path):
    """A liveness probe running concurrently with a real session's own
    ...
    proberos = [threading.Thread(target=probe_loop) for _ in range(4)]
    for t in proberos:
        t.start()
    try:
        for _ in range(50):
            try:
                session = store.create(session_id="probe-target")
                session.close()
                store.delete("probe-target")
