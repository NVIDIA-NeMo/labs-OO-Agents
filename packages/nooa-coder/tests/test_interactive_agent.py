...
def test_rename_session_without_a_session_handle_is_an_error(agent):
    with pytest.raises(RuntimeError, match="does not provide session renaming"):
        agent.rename_session("Some title")
