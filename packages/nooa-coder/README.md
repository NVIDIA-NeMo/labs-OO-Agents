...
The NOOA coding agent: the `CodingAgent` engine with its repository tools,
durable sessions, and an [Agent Client Protocol](https://agentclientprotocol.com)
(ACP) server. It replaces the separate `nooa-acp` package, which is now an
empty package that depends on `nooa-coder`.

| Module | Contents |
|---|---|
| `nooa_coder.coding` | `CodingAgent`, repository instructions, coding settings, slash commands |
| `nooa_coder.tools` | `RepoTools` (installed as the `nemo.repo` skill) and the `pyp` pipeline helpers |
| `nooa_coder.sessions` | durable session identity, metadata, and conversation replay |
| `nooa_coder.interactive_agent` | `InteractiveAgent`, the dispatcher-driven agent base class |
| `nooa_coder.acp` | the ACP server |

## ACP server

**Run the NOOA coding agent inside your editor.** The ACP server lets any
ACP-speaking client — Zed today — drive the coding agent: CodeAct, repository
tools, a persistent shell, installed skills, workspace slash commands and
durable sessions, with file edits and terminal commands surfaced as structured
activity.

It hosts `nooa_coder.coding.CodingAgent` directly. Repository instructions
...
The server is installed as two console scripts, `nooa-coder` and `nooa-acp`.
They are the same program; `nooa-acp` is kept so existing editor
configurations continue to work. With `nooa-cli` installed, it is also
available as `nooa acp`.

### Install

```bash
uv add nooa-coder               # or: uv add "nooa[coder]"

# ...with tree-sitter grammars for AST-aware RepoTools
uv add "nooa-coder[ast]"

# ...with numpy/pandas/plotly/scipy/sklearn pre-loaded into the LLM REPL
uv add "nooa-coder[datascience]"
```
...
```bash
uv run --project "$PWD" --package nooa-coder -- nooa-acp
```
...
session API: the live-session registry is isolated inside `nooa_coder.sessions.runtime` so it can
...
`nooa_coder.sessions` owns durable coding-agent session identity, metadata, and
conversation replay shared by hosts such as ACP. The process running an agent
owns the writable session handle; other hosts attach through their transport or
use read-only discovery. Generic event and SQLite storage primitives remain in
the core `nooa` package.
