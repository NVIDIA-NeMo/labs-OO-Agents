...
from nooa_coder.interactive.runtime import JobSnapshot
from nooa_coder.interactive.session_title import SessionTitleRequest
from nooa_coder.interactive.state import (
    AgentJobState,
    AgentJobSummary,
    AgentLifecycle,
    AgentState,
...
class LocalAgentRunner:
    ...
    def _on_done(self, task: asyncio.Future[Any]) -> None:
        # request_cancel()/_request_cancel_on_owner() read and write these
        # same fields under _lifecycle_lock (an RLock, so re-entering it
        # below via _set_lifecycle()/_finish_foreground() is safe) from
        # potentially another thread; mutating them here without the lock
        # let a concurrent cancel request observe a torn, inconsistent view
        # (e.g. _task already None but _cancel_requested still True).
        with self._lifecycle_lock:
        ...
        with self._lifecycle_lock:
            ...
            try:
                if source is not None:
                    ...
                    if not self._source_future.cancel():
                        raise RuntimeError("agent turn is no longer cancellable")
                else:
                    task.cancel()
            except RuntimeError:
                # Mirrors request_cancel()'s rollback: without it,
                # _cancel_requested stays permanently True (call_soon_
                # threadsafe can raise RuntimeError on a closing loop, e.g.
                # during a swap_agent()/shutdown race; future.cancel() can
                # return False for an already-done future), and the early
                # "if self._cancel_requested: return True" guard above then
                # makes every later cancel request a silent no-op forever.
                self._cancel_requested = False
                self._notify_cancelled = False
                return False
        self._changed()
        ...
        async def shutdown() -> None:
            ...
            if flush:
                for channel in queue_manager.channels().values():
                    if channel.mode == "queue":
                        # fire_on_get=True: a channel's on_get hook can have
                        # a durability side effect (e.g. user_messages
                        # recording an accepted prompt). Without it, an item
                        # admitted into the queue but flushed here before
                        # the dispatcher ever dequeued it -- a cancel racing
                        # a just-submitted prompt -- vanished with no trace.
                        channel.flush(fire_on_get=True)
