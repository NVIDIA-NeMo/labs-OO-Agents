...
async def test_behavior_controls_match_native_without_generating_turns(workspace):
    ...
    try:
        for name, args in [
            ("skills", ["deactivate", "parity.fixture"]),
            ("skills", ["activate", "parity.fixture"]),
        ]:
        ...
        for agent, sid in ((result.agent, result.session_id), (session.agent, created.session_id)):
            assert "parity.fixture" in agent.skills.activated()
        ...
        assert {"skills", "mcp"} <= {c.name for c in session.commands.commands()}
        assert not {"memory", "reflection"} & {c.name for c in session.commands.commands()}
    finally:
        await adapter.close()
        await close_native(result)
...
@pytest.mark.parametrize("host", ["native", "acp"])
async def test_workspace_model_preference_applies_to_future_agents(workspace, host):
    from nooa_cli.interactive.options import SessionOptions
    ...
    try:
        ...
        previous_models = [
            current.workspace_settings.status()["current"]["model"]
            for current in (native_result.agent, acp_agent)
        ]
        settings.set_default_model("fixture-model-for-future")
        assert SessionOptions.load(workspace).default_model == "fixture-model-for-future"
        for current, model in zip((native_result.agent, acp_agent), previous_models, strict=True):
            status = current.workspace_settings.status()
            assert status["saved"]["default_model"] == "fixture-model-for-future"
            assert status["current"]["model"] == model
            assert not {"memory", "reflection"} & status["saved"].keys()
            assert not hasattr(current, "memory")
            assert not hasattr(current, "_tui_reflection_runner")
    finally:
        await adapter.close()
        await close_native(native_result)
