    ...
    def _make_import_error(
        ...
        if tier == "blocked":
            ...
            if module_name == "subprocess":
                msg += (
                    " subprocess is unavailable in this REPL; use the `shell` tool "
                    "for commands, for example `await self.shell.run(...)`. "
                    "Stop and do not probe process-launch alternatives; your next tool call "
                    "must retry the command through `await self.shell.run(...)`."
                )
        else:
            msg = (
