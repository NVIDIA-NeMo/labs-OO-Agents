...
The nooa runtime sets this before each UnifiedLLM call so
...
This works correctly as long as the model call and the callback both
execute in the *same* async task as the ``set_journal_payload`` call —
which is true for nooa's normal execution model
(``_build_messages`` → ``UnifiedLLM.acall`` → callback, all in the
same coroutine).
"""
...
def set_journal_payload(payload: JournalPayload | None) -> None:
    """Called by the runtime just before each UnifiedLLM call.
