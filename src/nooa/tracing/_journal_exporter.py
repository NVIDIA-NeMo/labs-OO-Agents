...
2. Installs a NOOA lifecycle sink that intercepts message lists
...
destination list rather than installing a separate native sink per endpoint.
This keeps call correlation and delivery atomic across all destinations.
...
class JournalExporter(SpanExporter):
    ...
    wire payload.  The native lifecycle sink handles message delivery separately.
    """
    ...
    def _install(self) -> MessageJournalCallback:
        from nooa.tracing._llm_journal import acquire_http_sink

        return acquire_http_sink(self._base_url)
    ...
    def shutdown(self) -> None:
        from nooa.tracing._llm_journal import flush_pending, release_http_sink

        flush_pending(timeout=30.0)
        release_http_sink(self._callback, self._base_url)
    ...
    def force_flush(self, timeout_millis: int = 30_000) -> bool:
        ...
        from nooa.tracing._llm_journal import flush_pending
