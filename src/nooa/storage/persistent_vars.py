...
``InteractiveAgent.v`` uses it too — ``nooa.interactive.AgentVars`` is this same
class under its original name, so ``self.v`` and ``todo.v`` share one
implementation, including the ``get``/``set``/``items``/``keys``/``clear``
