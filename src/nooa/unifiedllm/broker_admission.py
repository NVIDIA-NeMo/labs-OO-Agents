...
class BrokerAdmissionSnapshot:
    """Current broker counters for diagnostics and comparison runs."""
...
class _BrokerState:
    ...
    def disconnect(self, waiter: _BrokerWaiter) -> None:
        ...
        if waiter.state == "queued":
            ...
            waiter.state = "cancelled"
            self._decrement_active(retract_reservation=True)
        elif waiter.state == "leased":
            waiter.state = "terminal"
            self._decrement_active(retract_reservation=False)
        self._offer_available()

    def confirm(self, waiter: _BrokerWaiter) -> dict[str, Any]:
        """Commit an accepted offer after the client acknowledges receipt."""
        if waiter.state != "offered":
            raise RuntimeError(f"Cannot confirm admission waiter in state {waiter.state!r}")
        waiter.state = "leased"
        if self.reserved_calls <= 0:
    ...
    def _decrement_active(self, *, retract_reservation: bool) -> None:
        if self.active <= 0 or (retract_reservation and self.reserved_calls <= 0):
            raise RuntimeError("Admission broker accounting underflow")
        self.active -= 1
        if retract_reservation:
            self.reserved_calls -= 1
    ...
    def _offer_available(self) -> None:
        while self.waiters:
            ...
            if (
                ...
                and self.admitted_calls + self.reserved_calls >= self.max_calls
            ):
                # An offered client may still disconnect before acknowledging
                # receipt. Keep later waiters queued until that reservation is
                # either committed or returned instead of rejecting them early.
                return
    ...
    async def _handle(
        ...
        try:
            ...
            while True:
                ...
                acquired = await reader.readline()
                if not acquired or len(acquired) > _MAX_MESSAGE_BYTES:
                    return
                if json.loads(acquired).get("status") != "acquired":
                    return
                confirmation = self.state.confirm(waiter)
                await self._write(writer, confirmation)
...
class BrokerAdmissionController:
    ...
    async def acquire(self, observer: AdmissionObserver) -> _BrokerAdmissionPermit:
        ...
        try:
            try:
                ...
                # Acknowledging ``ready`` proves that the offer reached this
                # client. The broker commits the total-call budget only after
                # this message, then confirms that commitment before provider
                # work may start.
                writer.write(b'{"status":"acquired"}\n')
                await self._drain(writer, started)
                third = await self._readline(reader, started)
                confirmation = json.loads(third) if third else {}
                if confirmation.get("status") != "confirmed":
                    raise AdmissionUnavailableError("Admission broker did not commit the lease")
                ...
                self._observe_terminal(observer, "timeout", {}, started)
                timeout_label = (
                ...
                self._observe_terminal(observer, "cancelled", {}, started)
                raise
    ...
    def _observe_terminal(
        self,
        observer: AdmissionObserver,
        outcome: Literal["timeout", "cancelled"],
        response: dict[str, Any],
        started: float,
    ) -> None:
        """Report timeout or cancellation without replacing that exception."""
        try:
            self._observe(observer, outcome, response, started)
