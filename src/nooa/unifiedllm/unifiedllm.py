...
from .types import (
    ...
    Tool,
    ToolCall,
    _clean_schema,  # noqa: F401 - private compatibility re-export
    _resolve_schema_refs,
    _strict_schema_valid,
    create_tool_from_callable,  # noqa: F401 - public compatibility re-export
)
...
try:
    ...
    _tracing = _sys.modules.get("nooa.tracing")
    if _tracing is not None:
        ensure_litellm_instrumented = getattr(_tracing, "ensure_litellm_instrumented", None)
        if ensure_litellm_instrumented is not None:
            ensure_litellm_instrumented()
except Exception:
    logger.debug("Could not install deferred LiteLLM tracing instrumentation", exc_info=True)
