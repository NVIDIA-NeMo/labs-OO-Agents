...
class JobHandle:
    ...
    async def cancel(self) -> None:
        ...
        If THIS handle already requested cancellation, wait for it without
        injecting another CancelledError into its cleanup. A job that
        ...
        A repeat request is skipped only when THIS handle already asked and
        the task still reports that request pending (cancelling() > 0).
        Neither signal alone is enough: cancelling() also counts cancels the
        job's own body triggered (an asyncio.timeout()/wait_for() it uses),
        so checking it alone can skip our very first request; tracking only
        our own flag would ignore a job that uncancel()ed and must accept a
        second request.
        """
        if self._task.done():
            try:
                ...
                pass
            return
        if not (self._cancel_called and self._task.cancelling()):
...
class Channel[T]:
    ...
    def flush(self, *, fire_on_get: bool = False) -> int:
        ...
        ``fire_on_get`` (default False, preserving the original plain-
        discard behavior for existing callers) also fires ``on_get`` for
        each discarded item, exactly as ``get()``/``drain()`` do for a
        genuinely consumed item. Without it, an item is either delivered
        or vanishes with no trace and no ``on_get`` firing at all -- for a
        channel whose ``on_get`` hook has a durability side effect (e.g.
        recording an accepted user message), a message admitted into the
        queue but flushed before the dispatcher ever dequeues it (a
        session/cancel racing a just-submitted prompt) was silently lost
        with nothing recorded, not even an error.
        """
        items = list(self._items) if fire_on_get else []
        n = self.qsize()
        self._items.clear()
        while self._waiters:
            waiter = self._waiters.popleft()
...
class QueueManager:
    ...
    def remove_channel(self, name: str) -> None:
        ...
        ch.flush()
        # Request cancellation but retain a still-cancelling handle until
        # shutdown can await its cleanup (this method is sync and cannot
        # await it itself). Spawn closures are bound to the concrete
        # channel and suppress stale output after a same-name replacement.
        ...
        # cancelling by now. Prune only handles that are both done AND for
        # this exact name -- never a once-off completed job elsewhere,
        # which handles()/UI history still needs to see. Without this,
        # repeated replace cycles on one channel name leaked one handle per
        # cycle for the life of the session.
        self._handles = [h for h in self._handles if not (h.name == name and h._task.done())]
    ...
    async def shutdown(self, *, keep_daemons: bool = False) -> None:
        ...
        # handle already asked and that request is still pending.
        for h in handles:
            if not h._task.done() and not (h._cancel_called and h._task.cancelling()):
                h._cancel_called = True
                h._task.cancel()
        for h in handles:
            await h.cancel()
