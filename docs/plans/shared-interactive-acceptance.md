...
> Historical development notes. The stacked upstream version defers long-term
> memory, reflection, and keep-going; commands for them below are not available.
> Custom agents must be selected explicitly with `--agent`, not settings files.
> Use [the current stack acceptance checks](stacked-tui-acceptance.md) for this revision.
