...
- Plain reasoning is stored locally by product decision, but external export is
  off by default.
- **Always-capture may not land until** log/repr, bug-report, clipboard, journal,
  OTLP, trace-download, and default event-explorer non-leak tests pass.
- The at-rest posture is explicit in config/docs before capture ships: reasoning
  uses the session store's existing filesystem protection, with no claim of
  encryption at rest; deployments needing stronger protection must use
  encrypted storage or disable local reasoning persistence by policy.
- Opaque bodies are hidden from repr, logs, exceptions, bug reports, clipboard,
  and normal event explorer summaries.
- Reasoning-aware redaction must not mistake ciphertext for harmless noise or
...
- Policy and provenance remain visible even when payload is redacted.
...
- reasoning body absent from default external exports.
