...
"""
Unit tests for ResponsesProjection and ResponsesClient._transform_messages.
...
def _project_rendered(messages):
    ...
    with ResponsesClient(model="openai/gpt-5.6") as client:
        wire, instructions = client._transform_messages(to_messages(messages))
    return ([{"role": "system", "content": instructions}] if instructions else []) + wire


class TestResponsesProjection:
    """Test that ResponsesProjection emits correct wire format."""
