...
def test_valid_unprojectable_sdk_turn_does_not_retain_partial_state(part) -> None:
    ...
    scope = replay_scope("openai/gpt-5.6", "responses", {})
    with pytest.raises(ReasoningReplayError, match="unsupported output") as error:
        capture_responses_state([reasoning, unsupported], scope)
    assert part in str(error.value)
    assert "provider-secret" not in str(error.value)
    assert "Not permitted." not in str(error.value)


def test_permuted_reasoning_indexes_are_not_replayed() -> None:
    from nooa.unifiedllm.replay_state import capture_responses_state
    ...
    with pytest.raises(ReasoningReplayError, match="Malformed OpenAI Responses"):
        prepare_responses_batch(public, state, scope)
