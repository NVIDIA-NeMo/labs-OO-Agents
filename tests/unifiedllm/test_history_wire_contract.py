...
def wire(monkeypatch, request):
    ...
    monkeypatch.setattr(
        DirectTransport,
        "_http_settings",
        staticmethod(lambda config: {"transport": httpx.MockTransport(respond)}),
    )
    monkeypatch.setattr(
        _ClientHttp,
        "_httpx_hardening",
...
@pytest.mark.parametrize("family", MODELS)
async def test_saved_turn_replays_after_switching_transport(family, wire, tmp_path, monkeypatch):
    import os
    ...
    turn.tag = "2"
    restored = _resume(turn, tmp_path / "transport-switch.db")
    previous = os.environ["NOOA_LLM_TRANSPORT"]
    monkeypatch.setenv("NOOA_LLM_TRANSPORT", "direct" if previous == "litellm" else "litellm")
    async with _client(family) as client:
        await _send(client, _render(restored, "after"))
    assert len(bodies) == 2
    _assert_replay(bodies[-1], family)
