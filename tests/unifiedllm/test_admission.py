...
async def test_completion_client_uses_injected_admission_controller():
    ...
    assert isinstance(client, UnifiedLLM)
    assert LLMCallContext(messages=[], client=client).client is client
    assert "admission_controller" not in base.config


@pytest.mark.asyncio
async def test_wrapper_is_per_use_and_delegates_runtime_attributes_with_fake_llm():
    events: list[str] = []
    ...
    class FakeLLM:
        ...
        async def acall(self, value: str) -> str:
            async def provider_attempt() -> str:
    ...
    assert await base.acall("plain") == "plain"
    assert await wrapped.acall("controlled") == "controlled"
    assert wrapped.model == "fake-model"
    assert wrapped.context_window == 4096
    assert events == ["provider:plain", "acquire", "provider:controlled", "release"]
...
def test_local_config_requires_explicit_limit():
    with pytest.raises(ValueError, match="max_in_flight is required"):
        AdmissionControlConfig(concurrency_group="shared", queue_timeout=1)


def test_named_group_does_not_require_http_endpoint():
    class FakeLLM:
        config = {"api_base": "provider-specific://socket"}

        async def acall(self) -> None:
            return None

    wrapped = AdmissionControl(
        FakeLLM(),
        AdmissionControlConfig(max_in_flight=1, concurrency_group="shared"),
    )

    assert isinstance(wrapped.controller, AdmissionPolicy)
    assert wrapped.controller.display_name == "shared"
...
def test_endpoint_identity_is_opaque_and_normalized():
    first = AdmissionPolicy(
        ...
        concurrency_group=None,
        queue_timeout=None,
        api_base="HTTPS://Gateway.Example:443/v1/?routing=blue#fragment",
    )
    ...
    assert "blue" not in first.display_name
...
@pytest.mark.asyncio
async def test_terminal_observer_failures_do_not_replace_timeout_or_cancellation():
    policy = _policy(1, group="terminal-observer", timeout=0.01)
    holder = await policy.acquire(lambda _detail: None)
    ...
    cancelled = asyncio.create_task(policy.acquire(fail_observation))
    await asyncio.sleep(0)
    cancelled.cancel()
    with pytest.raises(asyncio.CancelledError):
        await cancelled
...
async def test_dispatched_cancellation_holds_slot_until_provider_exits():
    ...
    second_task = asyncio.create_task(controlled(second_provider))
...
async def test_completion_and_responses_share_one_named_group():
    ...
    completion_base = CompletionClient("test-chat")
    responses_base = ResponsesClient("test-responses")
    completion = AdmissionControl(
        completion_base,
        AdmissionControlConfig(max_in_flight=2, concurrency_group="shared-gateway"),
    )
    responses = AdmissionControl(
        responses_base,
        AdmissionControlConfig(max_in_flight=2, concurrency_group="shared-gateway"),
    )
    try:
        ...
        await completion_base.aclose()
        await responses_base.aclose()
...
@pytest.mark.asyncio
async def test_endpoint_wrappers_join_one_inferred_budget():
    active = 0
    peak = 0
    ...
    owner_base = CompletionClient("owner", api_base="https://gateway.example/v1/")
    alias_base = CompletionClient("alias", api_base="https://GATEWAY.example:443/v1?ignored=true")
    owner = AdmissionControl(
        owner_base,
        AdmissionControlConfig(max_in_flight=1),
    )
    alias = AdmissionControl(
        alias_base,
        AdmissionControlConfig(max_in_flight=1),
    )
    try:
        ...
        await owner_base.aclose()
        await alias_base.aclose()
...
async def test_retry_reacquires_and_backoff_does_not_hold_slot():
    ...
    retrying_base = CompletionClient(
        "retry-model",
    ...
    other_base = CompletionClient(
        "other-model",
        retry_config=RetryConfig(max_retries=0, rate_limit_extra_retries=0),
    )
    retrying = AdmissionControl(
        retrying_base,
        AdmissionControlConfig(max_in_flight=1, concurrency_group="retry-group"),
    )
    other = AdmissionControl(
        other_base,
        AdmissionControlConfig(max_in_flight=1, concurrency_group="retry-group"),
    )
    try:
        ...
        await retrying_base.aclose()
        await other_base.aclose()
